class tabButtonSohle {
  ...
  tabButtonSohle(int xPos, int yPos, int numOfTabs, int myWidth, int myHeight, String[] mytextLabels) {
    this.xPos = xPos;
    this.yPos = yPos;
  ...
  void draw() {
    ...
    for (int i = 0; i < numOfTabs; i++) {
      ...
      fill(backgroundColor);
      textSize(20);
      text(mytextLabels[i], 0+11 + i * (myWidth)/numOfTabs, myHeight-5);
    }
    popMatrix();
